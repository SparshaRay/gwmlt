...
def get_remnant_properties(m1, m2, a1=0.0, a2=0.0, tilt1=0.0, tilt2=0.0, phi12=0.0) :
    ...
    Parameters:
        m1, m2 (float or np.ndarray): Component masses (in solar masses).
        a1, a2 (float, optional): Dimensionless spin magnitudes. Defaults to 0.
        tilt1, tilt2 (float, optional): Spin tilt angles relative to orbital angular momentum (rad). Defaults to 0.
        phi12 (float, optional): Difference in azimuthal angle between the two spins (rad). Defaults to 0.

    Returns:
        dict: Containing 'm_f' (final mass in solar masses) and 'a_f' (final dimensionless spin).
    """
...
def isco_radius(mass, spin) :
    ...
    Parameters:
        mass (float): mass of the black hole (the remnant) in solar masses.
        spin (float): dimensionless spin parameter of the black hole.

    Returns:
        dict: A dictionary containing 'prograde' and 'retrograde' ISCO radii in solar masses.
    """
...
def kerr_isco_freq(final_mass, final_spin) :
    ...
    Parameters:
        final_mass (float): Mass of the remnant black hole (in solar masses).
        final_spin (float): Dimensionless spin magnitude of the remnant black hole.

    Returns:
        dict: GW frequencies at ISCO for 'prograde' and 'retrograde' cases (in Hz).
    """
...
def bkl_isco_freq(m1, m2) :
    ...
    Parameters:
        m1, m2 (float or np.ndarray): Component masses of the binary (in solar masses).
        
    Returns:
        float or np.ndarray: Estimated GW frequency at ISCO (in Hz).
    """
...
def meco_freq(total_mass, q, spin1z, spin2z) :
    ...
    Parameters:
        total_mass (float or np.ndarray): Total mass of the binary (in solar masses).
        q (float or np.ndarray): Mass ratio (m_secondary / m_primary, where q <= 1).
        spin1z, spin2z (float or np.ndarray): Z-components of dimensionless spin for primary and secondary.

    Returns:
        float or np.ndarray: MECO frequency (in Hz).
    """
...
def ringdown_freq(m1, m2, a1=0.0, a2=0.0, tilt1=0.0, tilt2=0.0, phi12=0.0) :
    ...
    Returns:
        float: Ringdown frequency (in Hz).
    """
    remnant = get_remnant_properties(m1, m2, a1, a2, tilt1, tilt2, phi12)
...
def recommended_reference_freq(total_mass, q, spin1z, spin2z, f_ref=20.0, f_start=10.0, fudge_factor=0.97) :
    ...
    This is based on : asimov/pipelines/pe-configurator/peconfigurator/proc_samples.py#432
    
    Parameters:
        total_mass, q, spin1z, spin2z: Binary system parameters.
        f_ref (float): The default desired reference frequency in Hz (default 20.0).
        f_start (float): The starting frequency of the analysis (Hz) (default 10.0).
        fudge_factor (float): Safety margin applied to the MECO frequency (default 0.97).
